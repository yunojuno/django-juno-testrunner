...
class JunoDiscoverRunner(DiscoverRunner):
    """
    The only real difference between this and the standard
    DiscoverRunner in Django 1.6+ is the use of the custom
    TextTestRunner, which we hook in via run_suite()
    """
    def get_test_count(self, suite):
        """
        When running tests in parallel, a core suite is generated to
        contain a collection of sub-suites that are then ran in
        parallel.

        In that case we have to go and retrieve the count from each of
        the sub-suites as the parent will not directly contain the
        tests.
        """
